      ...
      <div v-else>
        <div v-for="level in presentLevels" :key="level" class="level-group">
          <div class="level-heading">
            <span class="level-badge">HSKK {{ level }}</span>
...
onMounted(async () => {
  loading.value = true
  exams.value = await listExams('hskk')
  loading.value = false
})
...
.level-badge { background: #1565c0; color: white; font-size: 0.9rem; font-weight: 700; padding: 4px 14px; border-radius: 20px; }
...
.btn-start { padding: 10px 24px; background: #1565c0; color: white; border: none; border-radius: 6px; font-weight: 600; cursor: pointer; white-space: nowrap; }
.btn-start:hover { background: #0d47a1; }
.btn-back { background: none; border: none; color: #1565c0; cursor: pointer; margin-bottom: 16px; font-size: 0.95rem; }
</style>
